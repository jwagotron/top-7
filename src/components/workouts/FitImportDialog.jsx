...
export default function FitImportDialog({ open, onClose, onImport }) {
  ...
  const handleFile = async (file) => {
    if (!file) return;
    if (typeof window !== 'undefined' && /^(localhost|127\.0\.0\.1)$/.test(window.location.hostname)) console.log('[UploadRun] upload started', { fileName: file.name, size: file.size });
    ...
    if (typeof window !== 'undefined' && /^(localhost|127\.0\.0\.1)$/.test(window.location.hostname)) console.log('[UploadRun] file type detected', ext);
    setError('');
    setStage('parsing');
    ...
    try {
      ...
      if (typeof window !== 'undefined' && /^(localhost|127\.0\.0\.1)$/.test(window.location.hostname)) console.log('[UploadRun] parse success', {
  ...
  const handleImport = () => {
    ...
    if (typeof window !== 'undefined' && /^(localhost|127\.0\.0\.1)$/.test(window.location.hostname)) console.log('[UploadRun] saving parsed run', { title: data.title, date: data.date, distance_km: data.distance_km, sport: data.sport });
    onImport(data);
    reset();
  };
  ...
  const handleManualSave = () => {
    ...
    if (typeof window !== 'undefined' && /^(localhost|127\.0\.0\.1)$/.test(window.location.hostname)) console.log('[UploadRun] saving manual run', data);
    onImport(data);
    reset();
  };
