...
export default function CompletionOverview({ plannedWorkouts = [], completions = [], athleteFilter, mStart, mEnd }) {
  ...
  if (typeof window !== 'undefined' && /^(localhost|127\.0\.0\.1)$/.test(window.location.hostname)) console.log('[CompletionOverview] range:', format(rangeStart,'yyyy-MM-dd'), '->', format(rangeEnd,'yyyy-MM-dd'));
  if (typeof window !== 'undefined' && /^(localhost|127\.0\.0\.1)$/.test(window.location.hostname)) console.log('[CompletionOverview] assigned:', assignedCount, '| completions prop:', completions.length, '| completed:', completed);
