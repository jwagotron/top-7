...
export default function JoinTeamModal({ open, onOpenChange, onSuccess }) {
  ...
  const handleJoin = async () => {
    ...
    setLoading(true);
    setError('');
    if (typeof window !== 'undefined' && /^(localhost|127\.0\.0\.1)$/.test(window.location.hostname)) console.log('[joinTeam] submit started', { code: teamCode.trim() });
    try {
      const response = await base44.functions.invoke('joinTeam', {
        invite_code: teamCode.toUpperCase().trim(),
      });
      const data = response.data;
      if (typeof window !== 'undefined' && /^(localhost|127\.0\.0\.1)$/.test(window.location.hostname)) console.log('[joinTeam] submit success', data);
